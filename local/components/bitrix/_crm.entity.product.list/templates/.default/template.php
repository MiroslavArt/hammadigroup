...
foreach ($grid['ROWS'] as $product)
{
	...
	// endregion QUANTITY
			...
			// region MARGIN
				  ...
				  $purchasePrice = $rawProduct['SKU_PROPERTIES']['PURCHASING_PRICE'];
if ($purchasePrice != 0) {
    $sellingPrice = $rawProduct['BASE_PRICE'];

    if ($sellingPrice != 0) {
        $profit = $sellingPrice - $purchasePrice;
        $marginValue = ($profit / $sellingPrice) * 100;
    } else {
        $marginValue = 0; // Handle case when selling price is zero
    }
} else {
    $marginValue = 100;
}
...
$product['MARGIN'] = round((float)$marginValue, $commonPrecision);



	// endregion MARGIN_RATE
