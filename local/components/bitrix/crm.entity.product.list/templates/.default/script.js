	...
	var StoreAvailablePopup = /*#__PURE__*/function () {
		...
		babelHelpers.createClass(StoreAvailablePopup, [{
			...
			value: function openDealsWithReservedProductSlider() {
				...
				//var dealId = babelHelpers.classPrivateFieldGet(this, _model).getField('DEAL_ID');  // Example: get the deal ID
				console.log('Deal ID:', storeId);
	...
	var Row = /*#__PURE__*/function () {
		...
		babelHelpers.createClass(Row, [{
			...
			value: function getCalculateFields() {
				return {
					...
					'QUOTE_PRICE_INITIAL': this.getQUOTE_PRICE_INITIAL(),
			...
			value: function getMargin() {
				return this.getField('MARGIN', 0);
			}
		}, {
			key: "getQUOTE_PRICE_INITIAL",
			value: function getQUOTE_PRICE_INITIAL() {
				return this.getField('QUOTE_PRICE_INITIAL', 0);
			}
		}, {
			...
			value: function updateFieldValue(code, value) {
				...
				switch (code) {
					...
					case 'COMMON_STORE_AMOUNT':
						...
						break;
					case 'MARGIN':
						this.changeMargin(value, mode);
						break;
					case 'QUOTE_PRICE_INITIAL':
						this.changeQUOTE_PRICE_INITIAL(value, mode);
						break;
			...
			key: "changeMargin",
			value: function changeMargin(value) {
				var mode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : MODE_SET;
				var preparedValue = this.parseInt(value);
				this.setMargin(preparedValue, mode);
			}
		},
		{
			key: "changeQUOTE_PRICE_INITIAL",
			value: function changeQUOTE_PRICE_INITIAL(value) {
				var mode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : MODE_SET;
				var preparedValue = this.parseInt(value);
				this.setQUOTE_PRICE_INITIAL(preparedValue, mode);
			}
		}, {
			key: "changeQuantity",
		...
		},
		{
			...
			value: function changeBasePrice(value) {
				...
				// Ensure the price is editable in edit mode
				...
				// Update UI input field in SET mode
				if (mode === MODE_SET) {
					this.updateUiInputField('PRICE', value.toFixed(this.getPricePrecision()));
				}
				...
				if (isChangedValue) {
					let purchasingPrice = this.getField('PURCHASING_PRICE');
					let qutePrice = this.getField('QUOTE_PRICE_INITIAL');
					qutePrice = Number(qutePrice);

					const categoryName = sessionStorage.getItem('CATEGORY_NAME');
					if (Number(purchasingPrice) > 0 && categoryName === 'Approved Cars') {
						let newMargin = Math.round((value - purchasingPrice) / (purchasingPrice / 100));
						this.setField('MARGIN', newMargin);
					}else if (Number(qutePrice) > 0 && categoryName === 'General'){
						let newMargin = Math.round((value - qutePrice) / (qutePrice / 100));
						this.setField('MARGIN', newMargin);
					}

					// Recalculate fields based on new base price
					var calculatedFields = this.getCalculator().calculateBasePrice(value);
					this.setFields(calculatedFields);

					// Refresh UI for all fields except BASE_PRICE and PRICE in edit mode
				...
				_classPrivateMethodGet$2(this, _togglePriceHintPopup, _togglePriceHintPopup2).call(
					this,
					originalPrice < 0 && originalPrice !== value
				);
			}
		},
		{
			...
			value: function setQuantity(value) {
				...
				if (isChangedValue) {
					...
					// Recalculate fields based on new quantity
					var calculatedFields = this.getCalculator().calculateQuantity(value);
					this.setFields(calculatedFields);

					// Refresh layout and trigger necessary actions
		...
		{
			key: "setMargin",
			value: function setMargin(value) {
				var mode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : MODE_SET;

				var isChangedValue = this.getField('MARGIN') !== value;

				// Update UI input field in SET mode
				if (mode === MODE_SET) {
					this.updateUiInputField('MARGIN', value);
				}

				if (isChangedValue) {
					this.setField('MARGIN', value);
					...
					if (Number(purchasingPrice) > 0 && categoryName === 'Approved Cars') {
						let basePrice = purchasingPrice + (purchasingPrice * value / 100);

						// Debugging: Check base price
						console.log('basePrice (Approved Cars):', basePrice);

						// Recalculate fields based on new price
						var calculatedFields = this.getCalculator().calculatePrice(basePrice);
						this.setFields(calculatedFields);
					} else if (Number(qutePrice) > 0 && categoryName === 'General') {
						let basePrice = qutePrice + (qutePrice * value / 100);


						var calculatedFields = this.getCalculator().calculatePrice(basePrice);
						this.setFields(calculatedFields);
					} else {
						console.error('Invalid values or category');
					}


					// Refresh layout and trigger necessary actions
					this.refreshFieldsLayout(['MARGIN']);
					this.addActionProductChange();
					this.addActionUpdateTotal();
				}
			}
		},
		{
			...
			key: "setQUOTE_PRICE_INITIAL",
			value: function setQUOTE_PRICE_INITIAL(value) {
				const categoryName = sessionStorage.getItem('CATEGORY_NAME');
				var mode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : MODE_SET;

				var isChangedValue = this.getField('QUOTE_PRICE_INITIAL') !== value;

				if (mode === MODE_SET) {
					this.updateUiInputField('QUOTE_PRICE_INITIAL', value);
				}

				if (isChangedValue) {
					this.setField('QUOTE_PRICE_INITIAL', value);

					// Now directly using QUOTE_PRICE_INITIAL for calculation
					let quotePriceInitial = value;
					if (categoryName == 'General') {
						// Assuming the base price is the QUOTE_PRICE_INITIAL value itself (no use of PURCHASING_PRICE)
						let basePrice = quotePriceInitial;

						// Recalculate fields based on the new base price
						var calculatedFields = this.getCalculator().calculatePrice(basePrice);
						this.setFields(calculatedFields);
					}
					// Refresh layout and trigger necessary actions
					this.refreshFieldsLayout(['QUOTE_PRICE_INITIAL']);
					this.addActionProductChange();
					this.addActionUpdateTotal();
				}
			}
	...
	var Editor = /*#__PURE__*/function () {
		...
		babelHelpers.createClass(Editor, [{
			...
			value: function handleOnEntityUpdate(event) {
				...
				console.log(data.entityData.CATEGORY_NAME);
				sessionStorage.setItem('CATEGORY_NAME', data.entityData.CATEGORY_NAME);
			}
		}, {
