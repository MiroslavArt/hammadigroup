    ...
    implements Main\Engine\Contract\Controllerable, Main\Errorable
{
    ...
    protected function getGridColumnsDescription(): array
    {
        ...
        $result['DISCOUNT_ROW'] = [
            ...
            'editable' =>
                !$this->isAllowedDiscount() || $this->isAllowedEditMargin()
                    ? false
                    : [
                    'TYPE' => Types::MONEY,
                    'CURRENCY_LIST' => $this->getCurrencyListForMoneyField(),
        ...
        if ($this->arParams['ENTITY_TYPE_NAME'] == 'DEAL') {

            $result['MARGIN'] = [
                'id' => 'MARGIN',
                'name' => Loc::getMessage('CRM_ENTITY_PRODUCT_LIST_COLUMN_MARGIN'),
                'width' => $columnDefaultWidth,
                'sort' => 'MARGIN',
                'align' => 'center',
                'editable' => !$this->isAllowedEditMargin() ? false : ['PLACEHOLDER' => '0'],
            ];
            $result['QUOTE_PRICE_INITIAL'] = [
                'id' => 'QUOTE_PRICE_INITIAL',
                'name' => Loc::getMessage('CRM_ENTITY_PRODUCT_LIST_COLUMN_MARGIN'),
                'width' => $columnDefaultWidth,
                'sort' => 'QUOTE_PRICE_INITIAL',
                'align' => 'center',
                'editable' => !$this->isAllowedEditMargin() ? false : ['PLACEHOLDER' => '0.0'],
            ];
        ...
        if ($this->crmSettings['ALLOW_TAX'])
        {
            ...
            if ($this->isAllowedEditMargin()) {
                $result['TAX_RATE']['editable'] = false;
            }
            ...
            if ($this->isAllowedEditMargin()) {
                $result['TAX_INCLUDED']['editable'] = false;
            }
    ...
    private function isAllowedEditMargin()
    {
        global $USER;

        return in_array($USER->getId(), [1, 4, 5, 6, 7, 9]);
    }
    ...
    private function loadCatalog(array $items): void
    {
        ...
        foreach ($this->rows as &$row)
        {
            $rsData = \Uchet\ProductRowDealMarginTable::getList(['filter' => ['PRODUCT_ROW_ID' => $row['ID']]])->Fetch();

            if ($rsData) {
                $row['MARGIN'] = $rsData['MARGIN'];
                 $row['QUOTE_PRICE_INITIAL'] = $rsData['QUOTE_PRICE_INITIAL'];
            }
        }
    }
